    ...
    try {
        ...
        if (req.file.mimetype === 'text/csv') {
            fs.createReadStream(filePath)
                ...
                .on('end', () => distributeTasks(entries, agents, res,decodedId.id));
        } else {
...
async function distributeTasks(entries, agents, res, adminId) {
    try {
        ...
        const chunkSize = Math.floor(entries.length / agents.length);
        let remainder = entries.length % agents.length;
        let index = 0;

        for (const agent of agents) {
            const extraTask = remainder > 0 ? 1 : 0; // Distribute remaining tasks
            const assignedTasks = entries.slice(index, index + chunkSize + extraTask);
            index += chunkSize + extraTask;
            remainder--;
