...
router.post('/add-agent', async (req, res) => {
    ...
    const newAgent = new Agent({ name, email, mobile, password: hashedPassword, adminId: decodedId.id});
    await newAgent.save();
...
router.get('/count/:id', async (req, res) => {
    ...
    const newAgent = await  Agent.find({adminId: decodedId.id});
    res.json({ count:newAgent.length });
});
router.get('/:id', async (req, res) => {
    const agentId = req.params.id;
    const decodedId =jwt.decode(agentId,process.env.JWT_SECRET);r
    const newAgent = await  Agent.find({adminId: decodedId.id});
    res.json({ newAgent });
});
