    ...
    <div className="flex flex-col items-center min-h-screen bg-gray-100 p-6">
          ...
          {agents.length > 0 ? (
            agents.map((agent) => (
              <li
                ...
                key={agent._id}
                className="bg-gray-100 p-3 rounded-lg shadow-md flex justify-between cur items-center"
              >
...

      {
        currentAgentList.length>0 && (
          <div>
            <span className="text-gray-600">Added By: {currentAgentList[0]?.agentId?.name}</span>
            {
              currentAgentList.map((item)=>{
                return (
                  <div key={item._id} className="flex items-center justify-between px-4 py-2 border-b-1 border-gray-200">
